...
We will use that $\langle H^*,H^*\rangle= n^2$ and that the equalities $\|H^*\|_*=\|H\|=n$ hold  when there are only two clusters.
...
In general, the first inequality need not hold and we can split the entries $ij$ into two sets $U$ and $V$ where this inequality is valid or (strictly) fails respectively. More precisely, define 
...
Since $b(\Gamma,\delta)<a(\Gamma,\delta)$ there exist nonnegative scalars $\gamma_{ij}^{st}$ for $ij\in U$ and $st\in V$ which tell us how much of the amount in entry $ij\in U$ has to be moved to entry $st\in V$. These numbers satisfy the following inequalities:
...
we conclude that $\widetilde{\Gamma}_{ab}\leq \widetilde{C_{ab}}\leq 1$ for $a\neq b$. The first inequality follows from those in~(\ref{ineqs1}). The second inequality follows since for $ab\in V$ we have $\widetilde{C_{ab}}=\widetilde{\Gamma_{ab}}\leq 1$ and for $ab\in U$ we have $\widetilde{C_{ab}}\leq -\frac{2\delta}{n}\leq 1$. 
 ...
 \mv{PROBLEMA DIAGONAL: Querr\'iamos demostrar que $\widetilde{C}$ esta en el subdiferencial de $f$ en $A^*$. Nos falta chequear la diagonal. Creo que lo que podemos demostrar es que se permiten ciertos tipos de transportes en los que no se mueve demasiada masa. Mejor a\'un, sera que siempre existe un transporte \'util? Por \'util quiero decir qu la suma con $a=b$ sea peque\~na para todo v\'ertice $i$. Esa suma se puede interpretar as\'i: en un v\'ertice $a$ del grafo llamemos positivas a las aristas en $V$ y negativas a las aristas en $U$ (porque de $U$ quitamos masa y en $V$ la subimos. Lo que la suma mide es: cu\'anto sumamos en las aristas que tienen mas menos la cantidad que restamos (en valor absoluto) de las aristas que tienen menos. Eso podr\'ia pensarse como el "badness" de un v\'ertice. La suma de los badness de todos los v\'erices es el doble del badness total (porque toda arista tiene dos v\'ertices).
Asi que concluimos que el transporte es v\'alido (la matriz $C$ estar\'a en el subdiferencial) si y solo si para todo v\'ertice el badness del v\'ertice respecto al transporte $\gamma_{ij}^{st}$ cumple que 
$|b_v-\frac{2\delta}{n}|\leq 1$ 
...
This algorithm converges under very general circumstances, which can be easily checked for problem \ref{WRNNS} :
...
\label{Numerics}
Using the algorithm given in the previous section, we tested the recovery of graphs generated by the stochastic block model. To this end, we implemented an algorithm on Julia that creates random graph that are distributed according to the stochastic block model. The input parameters are the following:
\begin{enumerate}
...
We begin by testing our algorithm by generating $3$ observations of a random graph which is distributed according to the stochastic block model and running our algorithm for different values of $\delta$. Success of recovery will be evaluated by using the \textit{error of recovery}  which is given by the formula
\[\|A^*-\bar{A}\|_1\]
where $A^*$ is the adjacency matrix of the underlying cluster structure and $\bar{A}$ is the adjacency matrix of the recovered graph.
We present one such plot in \ref{fig:errorRecovery}. Notice that the true graph structure is recovered for certain values of $\delta$.
...
Choosing a correct $\delta$ is of the utmost importance. 
...
Fortunately, our simulations suggest that the region were on can choose $\delta$ to obtain perfect recovery grows as more observations are added: see figures on \ref{fig:differentNorms}.
\begin{figure}[h]
\centering
...
As $\delta$ varies, the recovered graph changes as well. Figure   \ref{fig:recupera} Shows the recovered graph for different values of $\delta$. Note that a graph recovered by our algorithm has not necessarily entries in $\{0,1\}$ and one might be tempted to round down the entries bellow certain threshold (say 0.4) and round up those above another threshold (say 0.6). However, it is not clear what we should do with entries between both thresholds. 
...
We present the heat maps of such matrices in \ref{fig:Variapq} for different values of $N$. 
...
We also include the error of recovery of the graph obtained by averaging the observations, which we call the mean graph.
...
Notice that the error of recovery of the mean graph decreases as more observations are considered. It is still far off the true cluster structure even for 7 observations, even though with 3 observations we were already able to recover the true cluster structure using the nuclear norm. 
  
An interesting question that needs to be addressed is that of the speed of convergence to the true cluster structure as $N$ grows. If our algorithm needed to many observations of the random graph to find the cluster structure, we would simply use the mean of the observations. The following figures shows that this is not the case (at least for the stochastic block model), and thus there is a real benefit by using the Wasserstein summarization algorithm.
...
Most importantly, we showed that a well studied problem in machine learning can be recast as a robust optimization problem using the Wasserstein metric. This approach is very promising, as it gives a principled approach to machine learning which oftentimes lacks a theoretical background (such as the trendy deep learning). For example, see \cite{ColumbiaAlvaro}. This work is a small contribution to the growing body of applications of optimal transport to machine learning.
