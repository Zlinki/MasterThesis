...
Let $h$ be a real valued convex function on a vector space $V$ with an inner product. The subdifferential of $h$ at a point $a\in {\rm dom}(h)$ is the closed convex set given by
...
\end{cases}
\]
In this language $C_{ij}$ belongs to the subdifferential of $f$ if and only if $\widetilde{\Gamma}_{ij}\leq \widetilde{C}_{ij}$ for $i\neq j$.

Our next Lemma gives sufficient conditions for the true cluster structure $A^*$ to be a minimizer of the proposed optimization problem. In order to describe it we introduce the following notation. 
